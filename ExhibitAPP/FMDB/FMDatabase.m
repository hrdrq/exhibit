...
static FMDatabase* gAPPActivityDB = nil;
static FMDatabase* gAPPMyFavoriteDB = nil;


// Never change database
// And opent the database
+ (FMDatabase*) AppDatabaseGetInternal:(FMDatabase*) db path:(NSString*) inPath
{
    if (nil != db) {
        return db;
    }
    if (nil == inPath) {
        NSLog(@"error: wrong parameter inPath in APPDatabaseGet");
        return nil;
    }
    if (nil != [db databasePath] &&
        FALSE == [inPath isEqualToString:[db databasePath]]) {
        [db close];
    }
    if (nil == (db = [FMDatabase databaseWithPath:inPath])) {
        NSLog(@"error cannot get fmdatabase right");
        return nil;
    }
    if (FALSE == [db open]) {
        NSLog(@"error cannot open in APPDatabaseGet");
        return nil;
    }
    return db;
}
...
+ (FMDatabase*) APPDatabaseGet:(NSString*) inPath {
    ...
    return [FMDatabase AppDatabaseGetInternal:gAPPActivityDB path:inPath];
}

+ (FMDatabase*) APPMyFavoriteDBGet:(NSString*) inPath {
    return [FMDatabase AppDatabaseGetInternal:gAPPMyFavoriteDB path:inPath];
}

+ (BOOL) APPDatabaseRemove {
    FMDatabase* db = [FMDatabase APPDatabaseGet:nil];
    if (nil == db) {
        NSLog(@"error cannot remove the database");
        return FALSE;
    }
    if (FALSE == [db close]) {
        return FALSE;
...
+ (BOOL) APPMyFavoriteDatabaseRemove {
    FMDatabase* db = [FMDatabase APPMyFavoriteDBGet:nil];
    if (nil == db) {
        NSLog(@"error cannot remove the database");
        return FALSE;
    }
