...
- (NSDate*)APPdateForColumn:(NSString*)columnName {
    return [self APPdateForColumnIndex:[self columnIndexForName:columnName]];
}

- (NSDate*)APPdateForColumnIndex:(int)columnIdx {
    
    if (sqlite3_column_type([_statement statement], columnIdx) == SQLITE_NULL || (columnIdx < 0)) {
        return nil;
    }
    
    return [NSDateHandler LocalDateGet:[self stringForColumnIndex:columnIdx] dateFormat: EXPO_DATE_FORMATTER];
}
